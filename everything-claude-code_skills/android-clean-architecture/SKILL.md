...
Android 和 KMP 项目的简洁架构模式。涵盖模块边界、依赖反转、UseCase/Repository 模式，以及使用 Room、SQLDelight 和 Ktor 的数据层设计。
...
├── core/                 # 共享工具、基类、错误类型
├── domain/               # UseCases、领域模型、Repository 接口（纯 Kotlin）
├── data/                 # Repository 实现、DataSources、DB、网络
...
每个 UseCase 代表一个业务操作。使用 `operator fun invoke` 实现简洁的调用点：
...
- 将数据库实体或 DTO 暴露给 UI 层——始终映射到领域模型
- 在 ViewModels 中放置业务逻辑——提取到 UseCases
- 使用 `GlobalScope` 或非结构化协程——使用 `viewModelScope` 或结构化并发
- 臃肿的 Repository 实现——拆分为专注的 DataSources
- 循环模块依赖——如果 A 依赖 B，B 绝不能依赖 A

## 参考

有关 UI 模式，请参阅技能：`compose-multiplatform-patterns`。
有关异步模式，请参阅技能：`kotlin-coroutines-flows`。
