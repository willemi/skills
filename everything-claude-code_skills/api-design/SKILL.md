...
422 Unprocessable Entity  — 语义无效（JSON 有效，数据无效）
429 Too Many Requests     — 超出速率限制

# 服务器错误
500 Internal Server Error — 意外失败（绝不暴露详情）
...
**优点**：易于实现，支持"跳转到第 N 页"
**缺点**：大偏移时慢（OFFSET 100000），与并发插入不一致
...
- [ ] 使用正确的 HTTP 方法（GET 用于读，POST 用于创建等）
- [ ] 返回合适的状态码（不是所有情况都用 200）
